...
$order_qty = 56; // int | Optional order quantity in units of the instrument (i.e. contracts, for spot it is the base currency in minor currency (e.g. XBt quantity for XBT)).
$simple_leaves_qty = 1.2; // double | Deprecated: simple orders are not supported after 2018/10/26
$leaves_qty = 56; // int | Optional leaves quantity in units of the instrument (i.e. contracts, for spot it is the base currency in minor currency (e.g. XBt quantity for XBT)). Useful for amending partially filled orders.
...
# **orderCancelAll**
> \Swagger\Client\Model\Order[] orderCancelAll($target_account_ids, $symbol, $filter, $text)
...
try {
    $result = $apiInstance->orderCancelAll($target_account_ids, $symbol, $filter, $text);
    print_r($result);
} catch (Exception $e) {
...
$simple_order_qty = 1.2; // double | Deprecated: simple orders are not supported after 2018/10/26
$order_qty = 56; // int | Order quantity in units of the instrument (i.e. contracts, for spot it is base currency in minor currency for spot (e.g. XBt quantity for XBT)).
$price = 1.2; // double | Optional limit price for 'Limit', 'StopLimit', and 'LimitIfTouched' orders.
$display_qty = 56; // int | Optional quantity to display in the book. Use 0 for a fully hidden order.
