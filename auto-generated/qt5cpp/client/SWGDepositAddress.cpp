/**
 * BitMEX API
 * ## REST API for the BitMEX Trading Platform  _If you are building automated tools, please subscribe to the_ _[BitMEX API RSS Feed](https://blog.bitmex.com/api_announcement/feed/) for changes. The feed will be updated_ _regularly and is the most reliable way to get downtime and update announcements._  [View Changelog](/app/apiChangelog)  -  #### Getting Started  Base URI: [https://www.bitmex.com/api/v1](/api/v1)  ##### Fetching Data  All REST endpoints are documented below. You can try out any query right from this interface.  Most table queries accept `count`, `start`, and `reverse` params. Set `reverse=true` to get rows newest-first.  Additional documentation regarding filters, timestamps, and authentication is available in [the main API documentation](/app/restAPI).  _All_ table data is available via the [Websocket](/app/wsAPI). We highly recommend using the socket if you want to have the quickest possible data without being subject to ratelimits.  ##### Return Types  By default, all data is returned as JSON. Send `?_format=csv` to get CSV data or `?_format=xml` to get XML data.  ##### Trade Data Queries  _This is only a small subset of what is available, to get you started._  Fill in the parameters and click the `Try it out!` button to try any of these queries.  - [Pricing Data](#!/Quote/Quote_get)  - [Trade Data](#!/Trade/Trade_get)  - [OrderBook Data](#!/OrderBook/OrderBook_getL2)  - [Settlement Data](#!/Settlement/Settlement_get)  - [Exchange Statistics](#!/Stats/Stats_history)  Every function of the BitMEX.com platform is exposed here and documented. Many more functions are available.  ##### Swagger Specification  [⇩ Download Swagger JSON](swagger.json)  -  ## All API Endpoints  Click to expand a section. 
 *
 * OpenAPI spec version: 1.2.0
 * Contact: support@bitmex.com
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 * Do not edit the class manually.
 */


#include "SWGDepositAddress.h"

#include "SWGHelpers.h"

#include <QJsonDocument>
#include <QJsonArray>
#include <QObject>
#include <QDebug>

namespace Swagger {

SWGDepositAddress::SWGDepositAddress(QString json) {
    init();
    this->fromJson(json);
}

SWGDepositAddress::SWGDepositAddress() {
    init();
}

SWGDepositAddress::~SWGDepositAddress() {
    this->cleanup();
}

void
SWGDepositAddress::init() {
    address = new QString("");
    m_address_isSet = false;
    memo = new QString("");
    m_memo_isSet = false;
}

void
SWGDepositAddress::cleanup() {
    if(address != nullptr) { 
        delete address;
    }
    if(memo != nullptr) { 
        delete memo;
    }
}

SWGDepositAddress*
SWGDepositAddress::fromJson(QString json) {
    QByteArray array (json.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonObject jsonObject = doc.object();
    this->fromJsonObject(jsonObject);
    return this;
}

void
SWGDepositAddress::fromJsonObject(QJsonObject pJson) {
    ::Swagger::setValue(&address, pJson["address"], "QString", "QString");
    
    ::Swagger::setValue(&memo, pJson["memo"], "QString", "QString");
    
}

QString
SWGDepositAddress::asJson ()
{
    QJsonObject obj = this->asJsonObject();
    QJsonDocument doc(obj);
    QByteArray bytes = doc.toJson();
    return QString(bytes);
}

QJsonObject
SWGDepositAddress::asJsonObject() {
    QJsonObject obj;
    if(address != nullptr && *address != QString("")){
        toJsonValue(QString("address"), address, obj, QString("QString"));
    }
    if(memo != nullptr && *memo != QString("")){
        toJsonValue(QString("memo"), memo, obj, QString("QString"));
    }

    return obj;
}

QString*
SWGDepositAddress::getAddress() {
    return address;
}
void
SWGDepositAddress::setAddress(QString* address) {
    this->address = address;
    this->m_address_isSet = true;
}

QString*
SWGDepositAddress::getMemo() {
    return memo;
}
void
SWGDepositAddress::setMemo(QString* memo) {
    this->memo = memo;
    this->m_memo_isSet = true;
}


bool
SWGDepositAddress::isSet(){
    bool isObjectUpdated = false;
    do{
        if(address != nullptr && *address != QString("")){ isObjectUpdated = true; break;}
        if(memo != nullptr && *memo != QString("")){ isObjectUpdated = true; break;}
    }while(false);
    return isObjectUpdated;
}
}

