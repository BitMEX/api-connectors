...
SWGQuoteApi::SWGQuoteApi() {}

SWGQuoteApi::~SWGQuoteApi() {}

SWGQuoteApi::SWGQuoteApi(QString host, QString basePath) {
    this->host = host;
...
SWGQuoteApi::quote_get(QString* symbol, QString* filter, QString* columns, SWGNumber* count, SWGNumber* start, bool reverse, QDateTime* start_time, QDateTime* end_time) {
    ...
    connect(worker,
            ...
            this,
            &SWGQuoteApi::quote_getCallback);

    worker->execute(&input);
}

void
SWGQuoteApi::quote_getCallback(SWGHttpRequestWorker * worker) {
    QString msg;
    ...
    QList<SWGQuote*>* output = new QList<SWGQuote*>();
    QString json(worker->response);
    QByteArray array (json.toStdString().c_str());
    QJsonDocument doc = QJsonDocument::fromJson(array);
    QJsonArray jsonArray = doc.array();
    auto wrapper = new SWGQObjectWrapper<QList<SWGQuote*>*> (output);
    wrapper->deleteLater();
    foreach(QJsonValue obj, jsonArray) {
        SWGQuote* o = new SWGQuote();
        QJsonObject jv = obj.toObject();
        QJsonObject * ptr = (QJsonObject*)&jv;
        o->fromJsonObject(*ptr);
        auto objwrapper = new SWGQObjectWrapper<SWGQuote*> (o);
    ...
    if (worker->error_type == QNetworkReply::NoError) {
        emit quote_getSignal(output);
    } else {
        emit quote_getSignalE(output, error_type, error_str);
        emit quote_getSignalEFull(worker, error_type, error_str);
    }
}
...
SWGQuoteApi::quote_getBucketed(QString* bin_size, bool partial, QString* symbol, QString* filter, QString* columns, SWGNumber* count, SWGNumber* start, bool reverse, QDateTime* start_time, QDateTime* end_time) {
    ...
    connect(worker,
            &SWGHttpRequestWorker::on_execution_finished,
            this,
            &SWGQuoteApi::quote_getBucketedCallback);

    worker->execute(&input);
}

void
SWGQuoteApi::quote_getBucketedCallback(SWGHttpRequestWorker * worker) {
    QString msg;
    QString error_str = worker->error_str;
    QNetworkReply::NetworkError error_type = worker->error_type;
    ...
    if (worker->error_type == QNetworkReply::NoError) {
        emit quote_getBucketedSignal(output);
    } else {
        emit quote_getBucketedSignalE(output, error_type, error_str);
        emit quote_getBucketedSignalEFull(worker, error_type, error_str);
    }
}
