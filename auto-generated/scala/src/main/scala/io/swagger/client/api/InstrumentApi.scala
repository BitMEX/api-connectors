...
class InstrumentApi(
   ...
   */
  def instrumentGetUsdVolume(symbol: Option[String] = None, columns: Option[String] = None): Option[List[StatsUSDBySymbol]] = {
    val await = Try(Await.result(instrumentGetUsdVolumeAsync(symbol, columns), Duration.Inf))
    await match {
      case Success(i) => Some(await.get)
      case Failure(t) => None
    }
   ...
   * @param symbol Filter by symbol. (optional)
   * @param columns Array of column names to fetch. (optional)
   * @return Future(List[StatsUSDBySymbol])
   */
  def instrumentGetUsdVolumeAsync(symbol: Option[String] = None, columns: Option[String] = None): Future[List[StatsUSDBySymbol]] = {
      helper.instrumentGetUsdVolume(symbol, columns)
  }
...
class InstrumentApiAsyncHelper(client: TransportClient, config: SwaggerConfig) extends ApiClient(client, config) {
  ...
  def instrumentGetUsdVolume(symbol: Option[String] = None,
    columns: Option[String] = None
    )(implicit reader: ClientResponseReader[List[StatsUSDBySymbol]]): Future[List[StatsUSDBySymbol]] = {
    // create path and map variables
    val path = (addFmt("/instrument/usdVolume"))
