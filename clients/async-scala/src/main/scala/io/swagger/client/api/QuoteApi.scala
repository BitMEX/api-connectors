...
class QuoteApi(client: TransportClient, config: SwaggerConfig) extends ApiClient(client, config) {

  def quote.get(symbol: Option[String] = None,
  ...
  def quote.getBucketed(binSize: Option[String] = None,
    ...
    if (binSize != null) binSize.foreach { v => queryParams += "binSize" -> v.toString }
