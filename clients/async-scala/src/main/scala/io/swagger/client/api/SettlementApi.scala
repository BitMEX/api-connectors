...
class SettlementApi(client: TransportClient, config: SwaggerConfig) extends ApiClient(client, config) {

  
  def settlement.get(symbol: Option[String] = None,
      ...
      )(implicit reader: ClientResponseReader[List[Settlement]]): Future[List[Settlement]] = {
    // create path and map variables
    val path = (addFmt("/settlement"))
