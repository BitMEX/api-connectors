...
# **quote_get_bucketed**
> list[Quote] quote_get_bucketed(bin_size=bin_size, symbol=symbol, filter=filter, columns=columns, count=count, start=start, reverse=reverse, start_time=start_time, end_time=end_time)
...
try: 
    # Get previous quotes in time buckets.
    api_response = api_instance.quote_get_bucketed(bin_size=bin_size, symbol=symbol, filter=filter, columns=columns, count=count, start=start, reverse=reverse, start_time=start_time, end_time=end_time)
    pprint(api_response)
except ApiException as e:
