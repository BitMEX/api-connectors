...
class ChatApi(val defBasePath: String = "https://localhost/api/v1",
   ...
   * @return List[Chat]
   */
  def chatGet (count: Number /* = 100 */, start: Number /* = 0 */, reverse: Boolean /* = true */, channelID: Double) : Option[List[Chat]] = {
...
if(String.valueOf(channelID) != "null") queryParams += "channelID" -> channelID.toString
  ...
  def chatGetChannels () : Option[List[ChatChannel]] = {
    // create path and map variables
    val path = "/chat/channels".replaceAll("\\{format\\}","json")
    val contentTypes = List("application/json", "application/x-www-form-urlencoded", "application/json")
    val contentType = contentTypes(0)
    ...
    try {
      apiInvoker.invokeApi(basePath, path, "GET", queryParams.toMap, formParams.toMap, postBody, headerParams.toMap, contentType) match {
        case s: String =>
           Some(ApiInvoker.deserialize(s, "array", classOf[ChatChannel]).asInstanceOf[List[ChatChannel]])
        case _ => None
      }
    } catch {
      case ex: ApiException if ex.code == 404 => None
   ...
   * @return Chat
   */
  def chatNew (message: String, channelID: Double /* = 1 */) : Option[Chat] = {
    // create path and map variables
    val path = "/chat".replaceAll("\\{format\\}","json")
    val contentTypes = List("application/json", "application/x-www-form-urlencoded", "application/json")
    val contentType = contentTypes(0)
    ...
    if(contentType.startsWith("multipart/form-data")) {
      ...
      mp.field("channelID", channelID.toString(), MediaType.MULTIPART_FORM_DATA_TYPE)
      
      postBody = mp
    }
    else {
      formParams += "message" -> message.toString()
      formParams += "channelID" -> channelID.toString()
    }
