...
class FundingApi(val defBasePath: String = "https://localhost/api/v1",
   ...
   * @return List[Funding]
   */
  def fundingGet (symbol: String, filter: String, columns: String, count: Number /* = 100 */, start: Number /* = 0 */, reverse: Boolean /* = false */, startTime: Date, endTime: Date) : Option[List[Funding]] = {
    // create path and map variables
    val path = "/funding".replaceAll("\\{format\\}","json")
    ...
    try {
      apiInvoker.invokeApi(basePath, path, "GET", queryParams.toMap, formParams.toMap, postBody, headerParams.toMap, contentType) match {
        case s: String =>
           Some(ApiInvoker.deserialize(s, "array", classOf[Funding]).asInstanceOf[List[Funding]])
