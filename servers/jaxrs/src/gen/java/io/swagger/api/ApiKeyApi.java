...
public class ApiKeyApi  {
   private final ApiKeyApiService delegate = ApiKeyApiServiceFactory.getApiKeyApi();
    ...
    public Response apiKeyDisable(@ApiParam(value = "API Key ID (public component).", required=true)@FormParam("apiKeyID")  String apiKeyID,@Context SecurityContext securityContext)
    throws NotFoundException {
        return delegate.apiKeyDisable(apiKeyID,securityContext);
    }
    @POST
    @Path("/enable")
    @Consumes({ "application/json", "application/x-www-form-urlencoded" })
    ...
    @io.swagger.annotations.ApiResponses(value = { 
        ...
        @io.swagger.annotations.ApiResponse(code = 404, message = "Not Found", response = ApiKey.class) })
    public Response apiKeyEnable(@ApiParam(value = "API Key ID (public component).", required=true)@FormParam("apiKeyID")  String apiKeyID,@Context SecurityContext securityContext)
    throws NotFoundException {
        return delegate.apiKeyEnable(apiKeyID,securityContext);
    }
    @GET
    ...
    public Response apiKeyGet(@ApiParam(value = "If true, will sort results newest first.", defaultValue="false") @DefaultValue("false") @QueryParam("reverse") Boolean reverse,@Context SecurityContext securityContext)
    throws NotFoundException {
        return delegate.apiKeyGet(reverse,securityContext);
    }
    @POST
    ...
    @io.swagger.annotations.ApiResponses(value = { 
        ...
        @io.swagger.annotations.ApiResponse(code = 404, message = "Not Found", response = ApiKey.class) })
    public Response apiKeyNew(@ApiParam(value = "Key name. This name is for reference only.")@FormParam("name")  String name,@ApiParam(value = "CIDR block to restrict this key to. To restrict to a single address, append \"/32\", e.g. 207.39.29.22/32. Leave blank or set to 0.0.0.0/0 to allow all IPs. Only one block may be set. <a href=\"http://software77.net/cidr-101.html\">More on CIDR blocks</a>")@FormParam("cidr")  String cidr,@ApiParam(value = "Key Permissions. All keys can read margin and position data. Additional permissions must be added. Available: [\"order\", \"withdraw\"].")@FormParam("permissions")  String permissions,@ApiParam(value = "Set to true to enable this key on creation. Otherwise, it must be explicitly enabled via /apiKey/enable.", defaultValue="false")@FormParam("enabled")  Boolean enabled,@ApiParam(value = "OTP Token (YubiKey, Google Authenticator)")@FormParam("token")  String token,@Context SecurityContext securityContext)
    throws NotFoundException {
        return delegate.apiKeyNew(name,cidr,permissions,enabled,token,securityContext);
    }
    @DELETE
    ...
    public Response apiKeyRemove(@ApiParam(value = "API Key ID (public component).", required=true)@FormParam("apiKeyID")  String apiKeyID,@Context SecurityContext securityContext)
    throws NotFoundException {
        return delegate.apiKeyRemove(apiKeyID,securityContext);
    }
}
