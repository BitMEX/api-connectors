...
public class ChatApi  {
   private final ChatApiService delegate = ChatApiServiceFactory.getChatApi();
    ...
    public Response chatGet(@ApiParam(value = "Number of results to fetch.", defaultValue="100") @DefaultValue("100") @QueryParam("count") BigDecimal count,@ApiParam(value = "Starting point for results.", defaultValue="0") @DefaultValue("0") @QueryParam("start") BigDecimal start,@ApiParam(value = "If true, will sort results newest first.", defaultValue="true") @DefaultValue("true") @QueryParam("reverse") Boolean reverse,@Context SecurityContext securityContext)
    throws NotFoundException {
        return delegate.chatGet(count,start,reverse,securityContext);
    }
    @GET
    @Path("/connected")
    @Consumes({ "application/json", "application/x-www-form-urlencoded" })
    ...
    public Response chatGetConnected(@Context SecurityContext securityContext)
    throws NotFoundException {
        return delegate.chatGetConnected(securityContext);
    }
    @POST
    ...
    public Response chatNew(@ApiParam(value = "", required=true)@FormParam("message")  String message,@Context SecurityContext securityContext)
    throws NotFoundException {
        return delegate.chatNew(message,securityContext);
    }
}
