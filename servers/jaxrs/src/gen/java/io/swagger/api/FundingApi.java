...
public class FundingApi  {
   private final FundingApiService delegate = FundingApiServiceFactory.getFundingApi();
    ...
    public Response fundingGet(@ApiParam(value = "Instrument symbol. Send a bare series (e.g. XBU) to get data for the nearest expiring contract in that series.\n\nYou can also send a timeframe, e.g. `XBU:monthly`. Timeframes are `daily`, `weekly`, `monthly`, `quarterly`, and `biquarterly`.") @QueryParam("symbol") String symbol,
    ...
    throws NotFoundException {
        return delegate.fundingGet(symbol,filter,columns,count,start,reverse,startTime,endTime);
    }
}
