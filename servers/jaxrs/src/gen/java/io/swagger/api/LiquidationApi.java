...
public class LiquidationApi  {
   private final LiquidationApiService delegate = LiquidationApiServiceFactory.getLiquidationApi();
    ...
    public Response liquidationGet(@ApiParam(value = "Instrument symbol. Send a bare series (e.g. XBU) to get data for the nearest expiring contract in that series.\n\nYou can also send a timeframe, e.g. `XBU:monthly`. Timeframes are `daily`, `weekly`, `monthly`, `quarterly`, and `biquarterly`.") @QueryParam("symbol") String symbol,
    ...
    throws NotFoundException {
        return delegate.liquidationGet(symbol,filter,columns,count,start,reverse,startTime,endTime);
    }
}
