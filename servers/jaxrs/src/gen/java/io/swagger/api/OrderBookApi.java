...
public class OrderBookApi  {
   private final OrderBookApiService delegate = OrderBookApiServiceFactory.getOrderBookApi();
    ...
    public Response orderBookGet(@ApiParam(value = "Instrument symbol. Send a series (e.g. XBT) to get data for the nearest contract in that series.",required=true) @QueryParam("symbol") String symbol,@ApiParam(value = "Orderbook depth.", defaultValue="25") @DefaultValue("25") @QueryParam("depth") BigDecimal depth,@Context SecurityContext securityContext)
    throws NotFoundException {
        return delegate.orderBookGet(symbol,depth,securityContext);
    }
    @GET
    @Path("/L2")
    @Consumes({ "application/json", "application/x-www-form-urlencoded" })
    ...
    public Response orderBookGetL2(@ApiParam(value = "Instrument symbol. Send a series (e.g. XBT) to get data for the nearest contract in that series.",required=true) @QueryParam("symbol") String symbol,@ApiParam(value = "Orderbook depth per side. Send 0 for full depth.", defaultValue="25") @DefaultValue("25") @QueryParam("depth") BigDecimal depth,@Context SecurityContext securityContext)
    throws NotFoundException {
        return delegate.orderBookGetL2(symbol,depth,securityContext);
    }
}
