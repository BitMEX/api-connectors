...
import com.wordnik.client.model.Funding
...
class FundingApi (implicit val swagger: Swagger) extends ScalatraServlet 
    with FileUploadSupport
    with JacksonJsonSupport
    with SwaggerSupport {
  protected implicit val jsonFormats: Formats = DefaultFormats

  protected val applicationDescription: String = "FundingApi"
  override protected val applicationName: Option[String] = Some("Funding")
  ...
  val funding.getOperation = (apiOperation[List[Funding]]("funding.get")
  ...
  get("/funding",operation(funding.getOperation)) {
