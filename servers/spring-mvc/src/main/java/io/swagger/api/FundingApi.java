...
@Controller
@RequestMapping(value = "/funding", produces = {APPLICATION_JSON_VALUE})
...
public class FundingApi {
  ...
  @ApiOperation(value = "Get funding history.", notes = "", response = Funding.class, responseContainer = "List")
  @ApiResponses(value = { 
  ...
  public ResponseEntity<List<Funding>> fundingGet(@ApiParam(value = "Instrument symbol. Send a bare series (e.g. XBU) to get data for the nearest expiring contract in that series.\n\nYou can also send a timeframe, e.g. `XBU:monthly`. Timeframes are `daily`, `weekly`, `monthly`, `quarterly`, and `biquarterly`.") @RequestParam(value = "symbol", required = false) String symbol
