...
@Controller
@RequestMapping(value = "/insurance", produces = {APPLICATION_JSON_VALUE})
...
public class InsuranceApi {
  ...
  @ApiOperation(value = "Get insurance fund history.", notes = "", response = Insurance.class, responseContainer = "List")
  @ApiResponses(value = { 
  ...
  public ResponseEntity<List<Insurance>> insuranceGet(@ApiParam(value = "Instrument symbol. Send a bare series (e.g. XBU) to get data for the nearest expiring contract in that series.\n\nYou can also send a timeframe, e.g. `XBU:monthly`. Timeframes are `daily`, `weekly`, `monthly`, `quarterly`, and `biquarterly`.") @RequestParam(value = "symbol", required = false) String symbol
...
,
    @ApiParam(value = "Ending date filter for results.") @RequestParam(value = "endTime", required = false) Date endTime
      ...
      // do some magic!
      return new ResponseEntity<List<Insurance>>(HttpStatus.OK);
  }
