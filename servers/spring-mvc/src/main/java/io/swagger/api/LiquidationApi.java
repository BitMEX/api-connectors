...
@Controller
@RequestMapping(value = "/liquidation", produces = {APPLICATION_JSON_VALUE})
...
public class LiquidationApi {
  ...
  @ApiOperation(value = "Get liquidation orders.", notes = "", response = Liquidation.class, responseContainer = "List")
  @ApiResponses(value = { 
  ...
  public ResponseEntity<List<Liquidation>> liquidationGet(@ApiParam(value = "Instrument symbol. Send a bare series (e.g. XBU) to get data for the nearest expiring contract in that series.\n\nYou can also send a timeframe, e.g. `XBU:monthly`. Timeframes are `daily`, `weekly`, `monthly`, `quarterly`, and `biquarterly`.") @RequestParam(value = "symbol", required = false) String symbol
