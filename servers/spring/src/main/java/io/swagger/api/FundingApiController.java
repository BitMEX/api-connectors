...
public class FundingApiController implements FundingApi {
    ...
    public ResponseEntity<List<Funding>> fundingGet(@ApiParam(value = "Instrument symbol. Send a bare series (e.g. XBU) to get data for the nearest expiring contract in that series.  You can also send a timeframe, e.g. `XBU:monthly`. Timeframes are `daily`, `weekly`, `monthly`, `quarterly`, and `biquarterly`.") @RequestParam(value = "symbol", required = false) String symbol,
        ...
        // do some magic!
        return new ResponseEntity<List<Funding>>(HttpStatus.OK);
    }
